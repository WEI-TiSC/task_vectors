...
def apply_attention_qk_scaling(
        ...
        rng=None,
        mode="log_uniform"  # 可选: "log_uniform" 或 "beta"
):
    """
    ...
    scale_min, scale_max: float
        alpha[i] 的随机取值范围 [scale_min, scale_max]
    rng:
        可选的随机数发生器, 若不传则用 Python 原生 random
    mode: str
        "log_uniform" - 采用对数均匀采样，使值更偏向 scale_min 或 scale_max
        "beta" - 采用 Beta 分布采样，让数值远离中心区域

    使用示例:
    --------
        apply_attention_qk_scaling(params, layer_idx=3, scale_min=0.5, scale_max=20.0, mode="log_uniform")
    """
    ...
    # 生成 alpha[i]，保证采样更偏向极端值
    if mode == "log_uniform":
        if rng is None:
            alpha = torch.exp(torch.empty(d_k).uniform_(np.log(scale_min), np.log(scale_max)))  # log-uniform 采样
        else:
            alpha = torch.exp(
                (np.log(scale_min) + (np.log(scale_max) - np.log(scale_min)) * torch.rand(d_k, generator=rng)))

    elif mode == "beta":
        a, b = 0.5, 0.5  # Beta(0.5, 0.5) 使得采样更偏向两端
        if rng is None:
            alpha = scale_min + (scale_max - scale_min) * torch.tensor(np.random.beta(a, b, size=d_k))
        else:
            alpha = scale_min + (scale_max - scale_min) * torch.distributions.Beta(a, b).sample((d_k,))

    else:
        raise ValueError("Unsupported mode. Use 'log_uniform' or 'beta'.")
...
def apply_attention_qkvw_scaling(
    ...
    """
    在指定的 resblock Attention(第 layer_idx 层)上，对 Q、K、V 及输出投影 W_O 进行正系数缩放，
    保证整个 Multi-Head Self-Attention (MHA) 计算过程保持等价。
    ...
    使用示例:
    --------
        apply_attention_scaling(params, layer_idx=3, scale_min=0.5, scale_max=20.0, mode="log_uniform")
    """
    ...
    W_in_proj = params[key_in_proj_weight]  # [3 * d_k, d_model]
    b_in_proj = params[key_in_proj_bias]  # [3 * d_k]
    W_out_proj = params[key_out_proj_weight]  # [d_model, d_model]

    d_k = W_in_proj.shape[0] // 3  # 每个头的维度
    Q_weight, K_weight, V_weight = W_in_proj[:d_k, :], W_in_proj[d_k:2 * d_k, :], W_in_proj[2 * d_k:, :]
    Q_bias, K_bias, V_bias = b_in_proj[:d_k], b_in_proj[d_k:2 * d_k], b_in_proj[2 * d_k:]

    # 2. 生成 alpha[i] 确保采样更偏向极端值
    if mode == "log_uniform":
        if rng is None:
            alpha = torch.exp(torch.empty(d_k).uniform_(np.log(scale_min), np.log(scale_max)))  # log-uniform 采样
        else:
    ...
    # 7. 写回 in_proj_weight 和 out_proj_weight
    W_in_proj[:d_k, :] = Q_weight
    W_in_proj[d_k:2 * d_k, :] = K_weight
    W_in_proj[2 * d_k:, :] = V_weight
    b_in_proj[:d_k] = Q_bias
    b_in_proj[d_k:2 * d_k] = K_bias
    b_in_proj[2 * d_k:] = V_bias
